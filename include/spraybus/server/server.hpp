...
class Server : public networking::server::Server<Server, ClientData>,
  ...
  public:
    Server(int port) : Base(port), common::ClassLogger("server") {
        LOG_INFO(this->logger(), "Server started on port {}", port);
    }

    ~Server() { LOG_INFO(this->logger(), "Server shutting down"); }
    ...
    void on_message(Client* client, std::span<std::byte> message) {
        ...
        case protocol::Type::publish: {
            ...
            auto fanout_payload =
                m_protocol_constructor.fanout(topic_key, payload);
            for (auto* other_client : this->clients()) {
                if (other_client->id() != client->id() &&
                    other_client->data().subscribed_topics.contains(
                        topic_key)) {
